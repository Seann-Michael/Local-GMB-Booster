...
const tests = [
  ...
  },
  {
    ...
    validate: (url, key) =>
      url && url.includes("supabase.co") && key && key.startsWith("eyJ"),
  },
  {
...
tests.forEach((test) => {
  ...
  test.vars.forEach((varName, index) => {
    ...
    const status =
      value && value !== "your-" && value !== "demo_" && value !== "ACdemo"
        ? value.length > 20
...
if (passedTests === totalTests) {
  ...
  if (
    ...
    process.env.VITE_SUPABASE_URL.includes("your-")
  ) {
  ...
  if (
    ...
    process.env.TWILIO_ACCOUNT_SID.startsWith("ACdemo")
  ) {
