...
export function convertToMeters(distance: number, unit: 'miles' | 'kilometers'): number {
  if (unit === 'miles') {
...
export function generateWaypoints(options: WaypointGenerationOptions): Waypoint[] {
  ...
  switch (pattern) {
    case 'circle':
      return [...waypoints, ...generateCircularWaypoints(center, count - 1, distanceBetween, unit)];
    case 'grid':
      return [...waypoints, ...generateGridWaypoints(center, count - 1, distanceBetween, unit)];
    case 'line':
      return [...waypoints, ...generateLinearWaypoints(center, count - 1, distanceBetween, unit)];
    default:
      return waypoints;
  }
}
...
function generateCircularWaypoints(
  ...
  for (let ring = 1; ring <= ringsCount && waypointIndex < count; ring++) {
    ...
    const pointsInRing = ring === 1 ? Math.min(6, count - waypointIndex) : Math.min(8 * ring, count - waypointIndex);
...
function generateGridWaypoints(
  ...
  for (let row = -halfGrid; row <= halfGrid && waypointIndex < count; row++) {
    for (let col = -halfGrid; col <= halfGrid && waypointIndex < count; col++) {
      ...
      const lngOffset = (col * spacingInMeters) / (111320 * Math.cos(toRadians(center.lat)));
...
function generateLinearWaypoints(
  ...
  for (let distance = spacingInMeters; waypointIndex < count; distance += spacingInMeters) {
...
function destinationPoint(start: Coordinate, distance: number, bearing: number): Coordinate {
...
export function toggleWaypoint(waypoints: Waypoint[], waypointId: string): Waypoint[] {
