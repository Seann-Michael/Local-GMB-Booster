...
export const GridOverlayMap: React.FC<GridOverlayMapProps> = ({
  ...
  const generateGridPattern = useCallback((
    ...
    const lngDegPerMeter = 1 / (111320 * Math.cos(centerPoint.lat * Math.PI / 180));
  ...
  const createGridLines = useCallback((points: GridPoint[]): google.maps.Polyline[] => {
    ...
    for (let col = 1; col <= gridSize; col++) {
      const colPoints = points.filter(p => p.id.slice(1) === col.toString());
  ...
  const updateGridLines = useCallback((points: GridPoint[]) => {
  ...
  // Create markers
  const createMarkers = useCallback((points: GridPoint[]) => {
    if (!map) return;
  ...
  const updateRanking = useCallback((ranking: number | null) => {
    if (!selectedPoint) return;
    
    setGridPoints(prev => prev.map(p => 
      p.id === selectedPoint ? { ...p, ranking } : p
    ));
  ...
  const gridStats = useMemo(() => {
    ...
    const rankedPoints = gridPoints.filter(p => p.ranking !== null && p.ranking !== undefined).length;
    const avgRanking = gridPoints
            ...
            {selectedPoint && (
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  placeholder="Rank"
                  value={gridPoints.find(p => p.id === selectedPoint)?.ranking || ''}
                  onChange={(e) => updateRanking(e.target.value ? parseInt(e.target.value) : null)}
                />
              </div>
            )}
