...
const PlatformCard: React.FC<PlatformCardProps> = ({ 
  ...
  const getButtonText = (flowState: OAuthFlowState) => {
    if (flowState.isConnected) return `Connected to ${flowState.accountInfo?.accountName || 'Account'}`;
    if (flowState.isLoading) return 'Connecting...';
  ...
  return (
    <Card className={`transition-all duration-200 hover:shadow-md ${flowState.isConnected ? 'ring-2 ring-green-500' : ''}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <div className={`w-2 h-2 rounded-full ${getStatusColor(flowState.isConnected ? 'connected' : flowState.isLoading ? 'connecting' : flowState.error ? 'error' : 'not_connected')}`} />
      ...
      <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-1">
            {platform.scopes.map(scope => (
              <Badge key={scope} variant="secondary" className="text-xs">
                {scope.replace('https://www.googleapis.com/auth/', '').replace('_', ' ')}
              </Badge>
            ))}
          </div>
        </div>
        ...
        {flowState.isConnected && flowState.accountInfo && (
          <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
            ...
            <p className="text-sm text-green-700 mt-1">
              Account: {flowState.accountInfo.accountName || flowState.accountInfo.email}
            </p>
          </div>
        )}
        ...
        <Button
          ...
          className="w-full"
        >
          {flowState.isLoading && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}
...
export const ClientOnboardingPortal: React.FC<ClientOnboardingPortalProps> = ({
  ...
  // Calculate progress
  const connectedCount = Object.values(flowStates).filter(state => state.isConnected).length;
  const totalPlatforms = session?.required_platforms.length || 0;
  const progressPercentage = totalPlatforms > 0 ? (connectedCount / totalPlatforms) * 100 : 0;
  ...
  useEffect(() => {
    const loadSession = async () => {
      try {
        ...
        const sessionResponse = await fetch(`/api/oauth/onboarding/${onboardingToken}`);
        ...
        const availablePlatforms = platformsData.data.filter((p: PlatformConfiguration) => 
        ...
        sessionData.data.required_platforms.forEach((platform: string) => {
          ...
          initialStates[platform] = {
            ...
            accountInfo: connectedPlatform?.account_name ? { accountName: connectedPlatform.account_name } : undefined,
            error: connectedPlatform?.status === 'error' ? 'Connection failed' : undefined,
            retryCount: 0
          };
        });
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      setFlowStates(prev => ({
        ...
        [platform]: {
          ...
          accountInfo: type === 'oauth_success' ? data : prev[platform].accountInfo,
          error: type === 'oauth_error' ? error : undefined,
          retryCount: type === 'oauth_error' ? prev[platform].retryCount + 1 : prev[platform].retryCount
        }
      }));
  ...
  const updateSessionStatus = async (platform: string | null, status: string, accountInfo?: any) => {
    try {
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold">
                  {session.branding_config.company_name || 'Platform Onboarding'}
                </h1>
                <p className="text-muted-foreground">
                  Hi {session.client_name}, let's connect your marketing platforms
                </p>
              </div>
            </div>
            <div className="text-right">
              <div className="text-sm text-muted-foreground">Progress</div>
              <div className="font-semibold">{connectedCount}/{totalPlatforms}</div>
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        {isExpired && (
          <Alert variant="destructive" className="mb-6">
            <AlertCircle className="h-4 w-4" />
            <AlertDescription>
              This onboarding link has expired. Please contact your agency for a new link.
            </AlertDescription>
          </Alert>
        )}
        ...
        {isCompleted && (
          <Alert className="mb-6 border-green-200 bg-green-50">
            ...
            <AlertDescription className="text-green-800">
              🎉 Congratulations! All platforms have been connected successfully. 
              Your agency now has access to manage your marketing accounts.
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold mb-4">Connect Your Marketing Platforms</h2>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Securely connect your marketing accounts to give {session.branding_config.company_name || 'your agency'} 
            {session.access_level === 'read_only' ? ' view access to your data' : ' management access to your accounts'}.
          </p>
          <Badge variant="secondary" className="mt-2">
            {session.access_level === 'read_only' ? 'Read Only Access' : 'Full Management Access'}
          </Badge>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          {platforms.map(platform => (
            <PlatformCard
              ...
              platform={platform}
              flowState={flowStates[platform.platform] || {
        ...
        <div className="border-t pt-8 text-center space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            ...
            This connection is secured with industry-standard OAuth 2.0 authentication
          </p>
        </div>
      </div>
    </div>
