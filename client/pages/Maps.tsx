          ...
          <Card className="mt-6">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {rankingResults.slice(0, 8).map((result, index) => (
                  <div
                    key={result.waypoint.id}
                    className="p-3 border rounded-lg hover:bg-muted/50 cursor-pointer"
                    onClick={() => setSelectedWaypoint(result.waypoint)}
                  >
                    ...
                    <div className="text-xs text-muted-foreground">
                      {result.competitors.length > 0 && (
                        <div>Top: {result.competitors[0].name}</div>
                      )}
                    </div>
                  </div>
                ))}
              ...
              {rankingResults.length > 8 && (
                <div className="mt-4 text-center">
                  <Button variant="outline" size="sm">
                    View All {rankingResults.length} Results
                  </Button>
                </div>
              )}
            </CardContent>
