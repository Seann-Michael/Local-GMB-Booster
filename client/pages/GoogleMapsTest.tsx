...
export default function GoogleMapsTest() {
  ...
  const runTests = async () => {
    ...
    addResult("Environment Variable", "pending", "Checking VITE_GOOGLE_MAPS_API_KEY...");
    const envKey = import.meta.env.VITE_GOOGLE_MAPS_API_KEY;
    if (envKey) {
      addResult("Environment Variable", "success", `Found: ${envKey.substring(0, 10)}...${envKey.substring(envKey.length - 6)}`);
    } else {
      addResult("Environment Variable", "error", "VITE_GOOGLE_MAPS_API_KEY not found");
    }

    // Test 2: API Key Function
    addResult("API Key Function", "pending", "Testing getGoogleMapsApiKey()...");
    const retrievedKey = getGoogleMapsApiKey();
    setApiKey(retrievedKey);
    if (retrievedKey) {
      addResult("API Key Function", "success", `Retrieved: ${retrievedKey.substring(0, 10)}...${retrievedKey.substring(retrievedKey.length - 6)}`);
    } else {
    ...
    addResult("API Key Validation", "pending", "Validating API key with Google...");
    try {
      const validation = await validateGoogleMapsApiKey(retrievedKey);
      if (validation.valid) {
        addResult("API Key Validation", "success", "API key is valid");
      } else {
        addResult("API Key Validation", "error", validation.error || "API key invalid", validation);
      }
    } catch (error: any) {
      addResult("API Key Validation", "error", `Validation failed: ${error.message}`, error);
    }

    // Test 4: Script Loading
    addResult("Script Loading", "pending", "Loading Google Maps JavaScript API...");
    try {
      await loadGoogleMapsAPI();
      addResult("Script Loading", "success", "Google Maps API script loaded successfully");
    } catch (error: any) {
      addResult("Script Loading", "error", `Script loading failed: ${error.message}`, error);
    }
    ...
    try {
      if (typeof google !== "undefined" && google.maps && google.maps.places) {
        ...
        service.getPlacePredictions(
          ...
          (predictions, status) => {
            if (status === google.maps.places.PlacesServiceStatus.OK && predictions) {
              addResult("Places API", "success", `Found ${predictions.length} predictions`);
            } else {
        ...
        addResult("Places API", "error", "Google Maps Places API not available");
      }
    } catch (error: any) {
      addResult("Places API", "error", `Places API test failed: ${error.message}`, error);
    }
  ...
  const runQuickConnection = async () => {
    ...
    try {
      ...
      if (success) {
        addResult("Connection Test", "success", "Google Maps connection successful");
      } else {
        addResult("Connection Test", "error", "Google Maps connection failed");
      }
    } catch (error: any) {
      addResult("Connection Test", "error", `Connection test error: ${error.message}`, error);
    }
  ...
  return (
    <AppLayout>
      <div className="container px-4 py-6 max-w-4xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-bold mb-2">Google Maps API Diagnostic</h1>
            ...
            <CardContent>
              <div className="flex gap-4">
                <Button 
                  ...
                  className="gap-2"
                >
                  {isRunning && <div className="h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent" />}
                  Run Full Tests
                </Button>
                <Button 
            ...
            <CardContent>
              <div className="space-y-4 text-sm">
                <div>
                  ...
                  <p className="text-muted-foreground">Ensure your API key allows requests from your Netlify domain (*.netlify.app or your custom domain).</p>
                </div>
                <div>
                  <h4 className="font-medium mb-2">2. Required APIs</h4>
                  <p className="text-muted-foreground">Enable "Maps JavaScript API" and "Places API" in Google Cloud Console.</p>
                </div>
                <div>
                  <h4 className="font-medium mb-2">3. Billing Account</h4>
                  <p className="text-muted-foreground">Ensure your Google Cloud project has a valid billing account attached.</p>
                </div>
                <div>
                  <h4 className="font-medium mb-2">4. Environment Variables</h4>
                  <p className="text-muted-foreground">Verify VITE_GOOGLE_MAPS_API_KEY is set in Netlify and redeploy after changes.</p>
                </div>
              </div>
            </CardContent>
          </Card>
