...
const testData = {
  ...
  mediaUpload: {
    ...
    fileType: 'image/jpeg',
    fileData: 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==', // 1x1 pixel PNG in base64
...
async function testEmailFunction(baseUrl) {
  ...
  try {
    const response = await makeRequest(`${baseUrl}/.netlify/functions/send-email`, 'POST', testData.email);
...
async function testSMSFunction(baseUrl) {
  ...
  try {
    const response = await makeRequest(`${baseUrl}/.netlify/functions/send-sms`, 'POST', testData.sms);
    console.log(`   Status: ${response.status}`);
    console.log(`   Response:`, response.data);
    return response.status === 200;
  } catch (error) {
...
async function testDataForSEOFunction(baseUrl) {
  ...
  try {
    const response = await makeRequest(`${baseUrl}/.netlify/functions/dataforseo-service`, 'POST', testData.dataforseo);
    console.log(`   Status: ${response.status}`);
    console.log(`   Response:`, response.data);
    return response.status === 200;
  } catch (error) {
...
async function testMediaStorageFunction(baseUrl) {
  ...
  // Test upload
  try {
    const uploadResponse = await makeRequest(`${baseUrl}/.netlify/functions/media-storage`, 'POST', testData.mediaUpload);
    console.log(`   Upload Status: ${uploadResponse.status}`);
    console.log(`   Upload Response:`, uploadResponse.data);
    
    // Test list
    const listResponse = await makeRequest(`${baseUrl}/.netlify/functions/media-storage?limit=5`, 'GET');
...
async function runTests() {
  ...
  if (passedCount < totalCount) {
    ...
    console.log('- For production testing: ensure environment variables are set in Netlify dashboard');
  }
}
...
if (process.argv.includes('--help') || process.argv.includes('-h')) {
  ...
  console.log('');
  console.log('Options:');
  console.log('  --local    Test against local Netlify Dev server (http://localhost:8888)');
  console.log('  --help     Show this help message');
  console.log('');
  console.log('Environment Variables:');
  console.log('  TEST_BASE_URL    Override the base URL for testing (default: https://your-site.netlify.app)');
  process.exit(0);
}
